...
/*
 * Receiver for DS-FTP protocol.
 * Correctly handles Stop-and-Wait & Go-Back-N.
 * Implements ACK loss simulation (ChaosEngine) and window buffering.
 */
...
public class Receiver {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 5) {
            System.out.println("Usage: java Receiver <senderIP> <senderACKPort> <rcvDataPort> <outputFile> <RN>");
            return;
        }
        ...
        while (true) {
            ...
            if (packet.getType() == DSPacket.TYPE_SOT) {
                ...
                ackCount++;
                if (!ChaosEngine.shouldDrop(ackCount, RN)) sendACK(socket, senderIP, senderAckPort, 0);
                else System.out.println("Receiver: Simulated ACK loss (SOT)");
                break;
            }
        }
        ...
        while (true) {
            ...
            if (packet.getType() == DSPacket.TYPE_EOT) {
                ...
                ackCount++;
                if (!ChaosEngine.shouldDrop(ackCount, RN)) sendACK(socket, senderIP, senderAckPort, packet.getSeqNum());
                else System.out.println("Receiver: Simulated ACK loss (EOT)");
                break;
            }
            ...
            if (packet.getType() == DSPacket.TYPE_DATA) {
                ...
                if (diff < windowSize && !buffer.containsKey(seq)) buffer.put(seq, packet);
                ...
                ackCount++;
                if (!ChaosEngine.shouldDrop(ackCount, RN)) sendACK(socket, senderIP, senderAckPort, ackNum);
                else System.out.println("Receiver: Simulated ACK loss for SEQ = " + ackNum);
            }
        }
    ...
    private static void sendACK(DatagramSocket socket, InetAddress ip, int port, int seq) throws Exception {
        DSPacket ack = new DSPacket(DSPacket.TYPE_ACK, seq, null);
        socket.send(new DatagramPacket(ack.toBytes(), DSPacket.MAX_PACKET_SIZE, ip, port));
        System.out.println("Receiver: Sent ACK = " + seq);
    }
